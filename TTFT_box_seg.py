...
OUT_DIR = Path("./Results/box_prompt_seg")
...
BOX_DIR = Path("./Prompt/box_prompt")
BOX_HAS_CLASS_ID = True     
BOX_CLASS_OFFSET = 0        
BOX_PAD_PCT    = 0.05       
BOX_MIN_PAD_PX = 2          
BIN_THRESH     = 0.5        
FUSION_USE_SCORE_DENSITY = False  
SIM_THRESHOLD       = 0.25
...
def _pad_xywh_keep_center(x, y, w, h, W, H, pad_pct=BOX_PAD_PCT, min_pad_px=BOX_MIN_PAD_PX):
    scale = 1.0 + pad_pct
    nw = max(w * scale, w + 2 * min_pad_px)
    nh = max(h * scale, h + 2 * min_pad_px)
    cx, cy = x + w / 2.0, y + h / 2.0
    nx, ny = cx - nw / 2.0, cy - nh / 2.0
    nx = max(0.0, nx); ny = max(0.0, ny)
    nw = min(nw, W - nx); nh = min(nh, H - ny)
    return nx, ny, nw, nh

def load_coco_xywh_boxes_for_image(image_path: str) -> List[Tuple[int, float, float, float, float]]:
    """
    Returns a list of (cls0, x, y, w, h) from BOX_DIR/<image_stem>.txt, clamped and padded.
    If BOX_HAS_CLASS_ID=False, cls0 defaults to 0.
    """
    txt = BOX_DIR / (Path(image_path).stem + ".txt")
    if not txt.exists():
        return []
    ...
    with open(txt, "r") as f:
        for line in f:
            ...
            w = max(1.0, min(w, W - x)); h = max(1.0, min(h, H - y))
            x, y, w, h = _pad_xywh_keep_center(x, y, w, h, W, H)
            boxes.append((cls0, x, y, w, h))
    return boxes

def _xywh_to_xyxy(x: float, y: float, w: float, h: float) -> np.ndarray:
    return np.array([x, y, x + w, y + h], dtype=np.float32)

def encode_box_to_prompts(image_np: np.ndarray, xyxy: np.ndarray):
    """Fallback path that builds prompt embeddings from a box and runs the mask decoder,
    for cases where predictor.predict(box=...) isn't available."""
    try:
        predictor.set_image(image_np)
        mask_input, unnorm_coords, labels, unnorm_box = predictor._prep_prompts(
            points=None, labels=None, box=xyxy[None, :], mask_logits=None, normalize_coords=True
        )
        sparse_embeddings, dense_embeddings = prompt_enc(points=None, boxes=unnorm_box, masks=None)
        return sparse_embeddings, dense_embeddings
    except Exception:
        H, W = image_np.shape[:2]
        b = torch.as_tensor(xyxy[None, :], dtype=torch.float32, device=DEVICE)
        b[:, [0, 2]] /= float(W)
        b[:, [1, 3]] /= float(H)
        sparse_embeddings, dense_embeddings = prompt_enc(points=None, boxes=b, masks=None)
        return sparse_embeddings, dense_embeddings

def predict_best_mask_from_box(image_np: np.ndarray, xyxy: np.ndarray) -> Tuple[np.ndarray, float]:
    """Returns (mask_bool, score_float) for the best mask among multimask outputs."""
    H, W = image_np.shape[:2]
    try:
        predictor.set_image(image_np)
        masks, scores, _ = predictor.predict(box=xyxy, multimask_output=True)
        masks = np.asarray(masks)  
        scores = np.asarray(scores).reshape(-1) if scores is not None else np.ones(masks.shape[0], dtype=np.float32)
    except Exception:
        sparse_embeddings, dense_embeddings = encode_box_to_prompts(image_np, xyxy)
    ...
    mask_bin = (masks[j] > BIN_THRESH)
    return mask_bin, float(scores[j])

t_retrievals, t_ttft, t_boxseg, t_totals = [], [], [], []
...
for qi, q_path in enumerate(query_paths, start=1):
        ...
        while updates_done < TTFT_UPDATES:
            ...
            while micro < TTFT_ACCUM_STEPS and tries < MAX_EMPTY_TRIES:
                with autocast_ctx():
                    ...
                    seg_loss_bits = (seg_loss.detach() / math.log(2)).item()
                    seg_loss_bpb  = seg_loss_bits / max(bytes_per_pixel, 1e-9)
                    pending_bpp_vals.append(seg_loss_bits)
                    pending_bpb_vals.append(seg_loss_bpb)
    ...
    boxes_xywh = load_coco_xywh_boxes_for_image(q_path)
    if not boxes_xywh:
        print(f"Warning: no box file found for {q_path}; skipping image.")
        continue
    ...
    for cls0, x, y, w, h in boxes_xywh:
        xyxy = _xywh_to_xyxy(x, y, w, h)
        mask_bin, score = predict_best_mask_from_box(q_img, xyxy)

        if BOX_HAS_CLASS_ID:
            cls_id = int(cls0) + BOX_CLASS_OFFSET  
        else:
            if len(prototypes) > 0:
                qf = feature_map_for_image(sam2_model, x_q)
                m_feat = mask_to_feat(mask_bin.astype(np.float32), qf.shape[-2], qf.shape[-1])
                vec_q = masked_mean(qf, m_feat)
        ...
        if FUSION_USE_SCORE_DENSITY:
            area = float(mask_bin.sum()) + 1e-6
            score_eff = score / (area ** 0.5)
        else:
            score_eff = score

        if RESOLVE_TIES_HIGHER:
            upd = mask_bin & (score_eff > score_map)
        else:
            upd = mask_bin & (score_map == -np.inf)
        score_map[upd] = score_eff
        class_map[upd] = cls_id

    t_boxseg_q = time.perf_counter() - t0
    ...
    t_boxseg.append(t_boxseg_q); t_totals.append(t_total)

    print(f"[{qi:03d}] {q_stem} -> {raw_mask_path.name}, {viz_mask_path.name} | "
          f"ret={t_retrieval:.3f}s, ttft={t_ttft_q:.3f}s, boxseg={t_boxseg_q:.3f}s, e2e={t_total:.3f}s")
...
def build_coco_gt_and_dt(
    ...
    for g in gts:
        ...
        rle = _rle_from_binary_mask(g.mask.astype(np.uint8))
        bbox = maskUtils.toBbox(rle).tolist()
        area = float(maskUtils.area(rle))
        annotations.append({
            "id": ann_id, "image_id": img_id, "category_id": int(g.cls),
            "segmentation": rle, "area": area, "bbox": [float(b) for b in bbox], "iscrowd": 0
        }); ann_id += 1
    coco_gt = COCO()
    coco_gt.dataset = {
        "images": images, "categories": categories, "annotations": annotations,
        "info": {"description": "E-waste GT", "year": datetime.now().year}
    }
    coco_gt.createIndex()
    ...
    for p in preds:
        ...
        coco_dt_list.append({
            "image_id": img_id, "category_id": int(p.cls), "segmentation": rle, "score": float(p.score)
        })
    return coco_gt, coco_dt_list
...
def extract_overall_from_stats(coco_eval: COCOeval) -> Dict[str, float]:
    s = coco_eval.stats
    return {
        "mAP_50_95": float(s[0]), "AP50": float(s[1]), "AP75": float(s[2]),
        "mAP_small": float(s[3]), "mAP_medium": float(s[4]), "mAP_large": float(s[5]),
    }

def per_class_from_precision(coco_eval: COCOeval, class_count: int) -> Dict[int, Dict[str, float]]:
    precis = coco_eval.eval["precision"]  
    if precis is None:
        return {i+1: {"mAP_50_95": 0.0, "AP50": 0.0, "AP75": 0.0} for i in range(class_count)}
    ...
    t50 = _find_iou(0.50); t75 = _find_iou(0.75)
    out = {}
    for k in range(class_count):
        pc = precis[:, :, k, a_idx, m_idx]
        v = {"mAP_50_95": _mean_valid(pc)}
        v["AP50"] = _mean_valid(precis[t50, :, k, a_idx, m_idx][None, :]) if t50 is not None else 0.0
        v["AP75"] = _mean_valid(precis[t75, :, k, a_idx, m_idx][None, :]) if t75 is not None else 0.0
        out[k+1] = v
    return out
...
if len(perimage_gt) == 0:
    ...
    print("\nStandard pixel-wise PRF1 (background excluded)")
    for c in CLASS_IDS:
        cname = CLASS_NAMES[c-1]
...
if len(ALL_GTS) == 0:
    ...
    coco_gt, coco_dt_list = build_coco_gt_and_dt(ALL_PREDS, ALL_GTS, CLASS_NAMES, query_paths)
    coco_eval, coco_dt = run_cocoeval(coco_gt, coco_dt_list, iouType="segm")
    overall = extract_overall_from_stats(coco_eval)
    per_class_ap = per_class_from_precision(coco_eval, NUM_CLASSES)
    print("\n=== COCOeval (segm) ===")
...
summary = {
    ...
    "timing_sec_mean": {
        ...
        "box_segmentation": float(_mean(t_boxseg)),
...
if len(perimage_gt) > 0:
    ...
    summary.setdefault("pixel_prf1", {})
    summary["pixel_prf1"].update({
        "macro": {"P": round(P_std_macro, 4), "R": round(R_std_macro, 4), "F1": round(F1_std_macro, 4)},
        "per_class": [
            {"id": int(c), "name": CLASS_NAMES[c-1],
             "P": None if P_std[c] is None else round(float(P_std[c]), 4),
             "R": None if R_std[c] is None else round(float(R_std[c]), 4),
             "F1": None if F1_std[c] is None else round(float(F1_std[c]), 4)}
            for c in CLASS_IDS
        ],
    })
...
BOUNDARY_TOL_PX = 2
if len(perimage_gt) > 0:
    _, bf_per_class, bf_pool_detail = compute_dataset_boundary_f1(
        [gt.astype(np.int32) for gt in perimage_gt],
        [pr.astype(np.int32) for pr in perimage_pred],
        CLASS_IDS,
        tol_px=BOUNDARY_TOL_PX
    )
    summary.setdefault("boundary_f1", {})
    summary["boundary_f1"].update({
        "tolerance_px": int(BOUNDARY_TOL_PX),
        "overall": {
            "F1": round(float(bf_pool_detail["F1"]), 4),
            "P":  round(float(bf_pool_detail["P"]), 4),
            "R":  round(float(bf_pool_detail["R"]), 4),
            "total_pred_boundary": int(bf_pool_detail["tot_pred_b"]),
            "total_gt_boundary":   int(bf_pool_detail["tot_gt_b"]),
        },
        "per_class": [
            {
                "id": int(c),
                ...
                "P":  None if bf_per_class[c]["P"]  is None else round(float(bf_per_class[c]["P"]),  4),
                "R":  None if bf_per_class[c]["R"]  is None else round(float(bf_per_class[c]["R"]),  4),
...
if 'coco_metrics' in locals() and coco_metrics is not None:
    overall = coco_metrics["overall"]
    per_cls = coco_metrics["per_class"]
    summary["instance_AP"] = {
        "AP50": round(float(overall["AP50"]), 4),
        "AP75": round(float(overall["AP75"]), 4),
        "mAP_50_95": round(float(overall["mAP_50_95"]), 4),
        "mAP_50_95_by_size": {
            "small":  round(float(overall["mAP_small"]), 4),
            "medium": round(float(overall["mAP_medium"]), 4),
            "large":  round(float(overall["mAP_large"]), 4),
        },
        "per_class": [
            {"id": int(c), "name": CLASS_NAMES[c-1],
             "AP50": round(float(per_cls.get(c, {}).get("AP50", 0.0)), 4),
             "AP75": round(float(per_cls.get(c, {}).get("AP75", 0.0)), 4),
             "mAP_50_95": round(float(per_cls.get(c, {}).get("mAP_50_95", 0.0)), 4)}
            for c in range(1, NUM_CLASSES+1)
        ]
    }
